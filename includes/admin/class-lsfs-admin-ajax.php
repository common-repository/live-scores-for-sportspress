...
class LSFS_Admin_AJAX {
    ...
    public function add_scorer() {
        ...
        $event = new LSFS_Live_Event( $event_id );
        ...
        if( ! isset( $players[ $team_id ][ $team_player_id ] ) ) {   
            ...
            // Making sure the player is added as last one.
            $players[ $team_id ] += array( $team_player_id => array() );
        ...
        $event->add_timeline( $team_id, $team_player_id, $performance, $team_player_minute, $players[ $team_id ][ $team_player_id ][ $performance ] );

        update_post_meta( $event_id, 'sp_players', $players );
        
        if ( $primary_performance === $performance ) {
