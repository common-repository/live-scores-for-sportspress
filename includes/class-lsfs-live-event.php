...
class LSFS_Live_Event extends SP_Event {
    ...
    /**
     * Adding timeline data to event
     *
     * @param integer $team_id Team ID.
     * @param integer $player_id Player ID.
     * @param string  $performance Performance string, example: goals.
     * @param integer $minute Minute for timeline.
     * @param integer $performance_count Performance count, such as 2 goals, so we know for which we enter the minute.
     * @return void
     */
    public function add_timeline( $team_id, $player_id, $performance, $minute, $performance_count ) {
        
		$timeline = (array) get_post_meta( $this->ID, 'sp_timeline', true );

        if ( ! isset( $timeline[ $team_id ] ) ) {
            $timeline[ $team_id ] = array();
        }

        if ( ! isset( $timeline[ $team_id ][ $player_id ] ) ) {
            $timeline[ $team_id ][ $player_id ] = array();
        }

        if ( ! isset( $timeline[ $team_id ][ $player_id ][ $performance ] ) ) {
            $timeline[ $team_id ][ $player_id ][ $performance ] = array();
        }

        for( $i = 0; $i < $performance_count; $i++ ) {
            if ( ! isset( $timeline[ $team_id ][ $player_id ][ $performance ][ $i ] ) ) {
                $timeline[ $team_id ][ $player_id ][ $performance ][ $i ] = "";
            }

            // Current.
            if ( ( $performance_count - 1 ) === $i ) {
                $timeline[ $team_id ][ $player_id ][ $performance ][ $i ] = $minute;
            }
        }

        update_post_meta( $this->ID, 'sp_timeline', $timeline );
    }
}